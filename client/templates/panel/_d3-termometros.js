...
crearTermometro = function (indice, nodo) {
  termometros["DHT11-" + indice] = new Termometro(indice, nodo);
}

function Termometro(indice, nodo) {
  var self = this;
  var HEIGHT = window.innerWidth * 0.25, WIDTH = window.innerHeight * 0.8;
  ...
  this.render = function () {
    ...
    var bulbGradient = defs.append("radialGradient")
      .attr("id", "bulbGradient" + config.codigo)
      .attr("cx", "50%")
      .attr("cy", "50%")
      .attr("r", "50%")
      .attr("fx", "50%")
    ...
    [this.config.minTemp, this.config.maxTemp].forEach(function (t) {
      ...
      var isMax = (t ==this.config. maxTemp),
        label = (isMax ? "C" : "F"),
      ...
      svg.append("line")
        .attr("id", label + "Line")
        .attr("x1", this.config.xoffset - this.config.tubeWidth / 2)
        .attr("x2", this.config.xoffset + this.config.tubeWidth / 2 + 8)
        .attr("y1", scale(t))
        .attr("y2", scale(t))
        .style("stroke", this.config.tubeBorderColor)
  ...
  this.actualizar = function (valor) {
